...
public class Field {
					...
					public void mousePressed(MouseEvent e) {
						...
						if(e.getButton()==1 && !tiles[iFinal][jFinal].isFlagged()) {
							...
							if(revealTile(iFinal, jFinal)) {
								JOptionPane.showMessageDialog(contentPane, "You've awoken an evil spirit!");
								reset();
							}
							
							// If the only hidden tiles remaining are mines, tell the user they won and reset
							if(checkForVictory()) {
								JOptionPane.showMessageDialog(contentPane, "You successfully cleared the field!");
								reset();
							}
	...
	public boolean checkForVictory() {
		...
		// Linear search the tiles, returning false if any are hidden but do not have a mine
		for(int i=0;i<fieldSize;i++)
			for(int j=0;j<fieldSize;j++)
